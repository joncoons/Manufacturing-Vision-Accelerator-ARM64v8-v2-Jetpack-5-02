...
class InsertInference():
    ...
    def create_record(self):
        ...
        with msql.connect(host="127.0.0.1", user="SA", password=f"{self.sql_pwd}", database=f"{self.sql_db}") as sql_conn:
            ...
            create_inference_tbl = """CREATE TABLE if not exists inference_data (id INT NOT NULL AUTO_INCREMENT, 
            ...
            create_detections_tbl = """CREATE TABLE if not exists detection_data (id INT NOT NULL AUTO_INCREMENT, 
            ...
            if self.detection_count > 0:
                for i in range(int(self.detection_count)):
                    ...
                    cursor.execute("INSERT into detection_data (tag_name, tag_id, probability, bbox, unique_id) VALUES (%s,%s,%s,%s,%s)", 
                            (
                            str(self.tag_name),
                            str(self.tag_id),
